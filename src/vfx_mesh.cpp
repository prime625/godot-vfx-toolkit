...
// ============================================================================
// LOOP SELECTION — multisel
// ============================================================================

void VFXMesh::get_ordered_edges_around_vertex(int vertex_id, std::vector<int>& out_edges) const {
    if (vertex_id < 0 || vertex_id >= (int)vertices.size()) return;
    vfx::HEVertex* v = vertices[vertex_id];
    if (!v || v->deleted || !v->halfedge) return;

    vfx::HEEdge* h = v->halfedge;
    do {
        if (!h || h->deleted) break;
        out_edges.push_back((int)h->id);
        if (!h->twin) break;
        h = h->twin->next;
        if (!h) break;
    } while (h != v->halfedge);
...
PackedInt32Array VFXMesh::select_edge_loop(int edge_id) const {
    if (edge_id < 0 || edge_id >= (int)edges.size()) return PackedInt32Array();
    ...
    if (v0 < 0 || v1 < 0) return PackedInt32Array();

    for (int dir = 0; dir < 2; dir++) {
        int current_v = (dir == 0) ? v0 : v1;
        int current_edge = edge_id;

        while (true) {
            int h_to_v = current_edge;
            int ce_v0, ce_v1;
            get_edge_endpoints(current_edge, ce_v0, ce_v1);
            if (ce_v1 != current_v) {
                vfx::HEEdge* e = edges[current_edge];
                if (!e || !e->twin) break;
                h_to_v = (int)e->twin->id;
            }

            std::vector<int> edges_around;
            get_ordered_edges_around_vertex(current_v, edges_around);

            auto it = std::find(edges_around.begin(), edges_around.end(), h_to_v);
            if (it == edges_around.end()) break;

            int idx = (int)(it - edges_around.begin());
            int n = (int)edges_around.size();
            if (n < 4 || n % 2 != 0) break;
...
PackedInt32Array VFXMesh::select_vertex_loop(int vertex_id) const {
    if (vertex_id < 0 || vertex_id >= (int)vertices.size()) return PackedInt32Array();
    vfx::HEVertex* v = vertices[vertex_id];
    if (!v || v->deleted) return PackedInt32Array();
    ...
    PackedInt32Array arr;
    for (int id : verts) arr.push_back(id);
    return arr;
}

PackedInt32Array VFXMesh::select_face_loop(int face_id) const {
    if (face_id < 0 || face_id >= (int)faces.size()) return PackedInt32Array();
    vfx::HEFace* f = faces[face_id];
    if (!f || f->deleted || !f->halfedge) return PackedInt32Array();

    std::vector<vfx::HEEdge*> f_edges;
