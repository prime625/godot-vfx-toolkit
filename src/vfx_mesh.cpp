...
static vfx::HEEdge* _get_opposite_edge_in_face(vfx::HEEdge* he) {
    ...
    do {
        if (cur == he) {
            vfx::HEEdge* opp = cur->next->next;
            return (opp && !opp->deleted) ? opp : nullptr;
        }
        cur = cur->next;
    } while (cur && cur != start);
...
PackedInt32Array VFXMesh::get_vertex_loop(int vertex_id) const {
    ...
    for (int nbr : nbrs) {
        ...
        if (edge_loop.size() > 2) {
            ...
            for (int i = 0; i < edge_loop.size(); i++) {
                int v0 = -1, v1 = -1;
                get_edge_endpoints(edge_loop[i], v0, v1);
                if (v0 >= 0) verts.insert(v0);
                if (v1 >= 0) verts.insert(v1);
            }
            for (int v : verts) result.push_back(v);
            return result;
        }
    }
