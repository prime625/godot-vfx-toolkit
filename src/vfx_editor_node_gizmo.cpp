...
void VFXEditorNode::gizmo_drag(const Vector3& ray_origin, const Vector3& ray_dir) {
    ...
    Vector3 hit;
    Vector3 origin = gizmo_transform.get_origin();
    bool is_global_object = (edit_mode == MODE_OBJECT && !gizmo_local);
    ...
    if (gizmo_mode == GIZMO_TRANSLATE) {
        ...
        if (gizmo_drag_axis <= GIZMO_Z) {
            Vector3 axis = is_global_object
                ? Vector3(gizmo_drag_axis == 0 ? 1.0f : 0.0f, gizmo_drag_axis == 1 ? 1.0f : 0.0f, gizmo_drag_axis == 2 ? 1.0f : 0.0f)
                : t.basis.get_column(gizmo_drag_axis).normalized();
        ...
        // Use START transform for normal so it doesn't drift during the drag
        Vector3 normal;
        if (gizmo_drag_axis == GIZMO_X)      normal = is_global_object ? Vector3(1,0,0) : gizmo_drag_start_transform.basis.xform(Vector3(1,0,0)).normalized();
        else if (gizmo_drag_axis == GIZMO_Y) normal = is_global_object ? Vector3(0,1,0) : gizmo_drag_start_transform.basis.xform(Vector3(0,1,0)).normalized();
        else if (gizmo_drag_axis == GIZMO_Z) normal = is_global_object ? Vector3(0,0,1) : gizmo_drag_start_transform.basis.xform(Vector3(0,0,1)).normalized();
        ...
        if (gizmo_drag_axis <= GIZMO_Z) {
            // Use SAME axis logic as gizmo_begin_drag: world axes for global, local columns for local
            Vector3 world_axis = is_global_object
                ? Vector3(gizmo_drag_axis == 0 ? 1.0f : 0.0f, gizmo_drag_axis == 1 ? 1.0f : 0.0f, gizmo_drag_axis == 2 ? 1.0f : 0.0f)
                : gizmo_drag_start_transform.basis.get_column(gizmo_drag_axis).normalized();
