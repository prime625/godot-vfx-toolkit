...
void VFXEditorNode::_update_gizmo_scale() {
    ...
    world_size = vfx::clampf(world_size, gizmo_min_size, gizmo_max_size);
...
void VFXEditorNode::_update_gizmo_visibility() {
    ...
    bool show = !gizmo_locked && (active_scene_node.is_valid() || has_mesh_selection || selected_bone >= 0);
...
int VFXEditorNode::on_touch_down(const Vector3& ray_origin, const Vector3& ray_dir, const Vector2& screen_pos) {
    ...
    if (scene.is_valid() && edit_mode == MODE_OBJECT) {
        ...
        if (selected_bone < 0) {
            ...
            // Empty click while a bone is selected -> deselect it
            clear_selection();
        }
        return -1;
    }
...
Transform3D VFXEditorNode::_get_armature_transform() const {
    if (scene.is_valid() && skeleton.is_valid()) {
        // Find the node that owns the displayed skeleton, regardless of selection
        Array all = scene->flatten_tree();
        for (int i = 0; i < all.size(); i++) {
            Ref<VFXSceneNode> n = all[i];
            if (n.is_valid() && n->get_skeleton() == skeleton) {
