...
static void _append_cylinder(PackedVector3Array& verts, PackedColorArray& cols, PackedInt32Array& idx,
    ...
    for (int i = 0; i <= segs; i++) {
        ...
        verts.push_back(from + off); cols.push_back(col);
        verts.push_back(to + off);   cols.push_back(col);
    }
    for (int i = 0; i < segs; i++) {
        int b = base + i * 2;
...
static void _append_ring(PackedVector3Array& verts, PackedColorArray& cols, PackedInt32Array& idx,
    ...
    for (int i = 0; i <= segs; i++) {
        ...
        if (has_prev) _append_cylinder(verts, cols, idx, prev_pos, pos, tube_radius, 4, col);
        prev_pos = pos; has_prev = true;
    }
}
...
static void _append_box_lines(PackedVector3Array& verts, PackedColorArray& cols, PackedInt32Array& idx,
                        const Vector3& center, float size, const Color& col) {
    float h = size * 0.5f;
    Vector3 corners[8] = {
    ...
    for (int i = 0; i < 12; i++) {
        int base = verts.size();
        verts.push_back(corners[edges[i][0]]); cols.push_back(col);
        verts.push_back(corners[edges[i][1]]); cols.push_back(col);
...
void VFXEditorNode::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("set_show_face_normals", "show"), &VFXEditorNode::set_show_face_normals);
    ClassDB::bind_method(D_METHOD("get_show_face_normals"), &VFXEditorNode::get_show_face_normals);
    ClassDB::bind_method(D_METHOD("set_show_vertex_normals", "show"), &VFXEditorNode::set_show_vertex_normals);
    ClassDB::bind_method(D_METHOD("get_show_vertex_normals"), &VFXEditorNode::get_show_vertex_normals);
    ClassDB::bind_method(D_METHOD("set_show_face_centers", "show"), &VFXEditorNode::set_show_face_centers);
    ClassDB::bind_method(D_METHOD("get_show_face_centers"), &VFXEditorNode::get_show_face_centers);
    ...
    ClassDB::bind_method(D_METHOD("extrude_selected", "distance"), &VFXEditorNode::extrude_selected);
    ClassDB::bind_method(D_METHOD("inset_selected", "amount"), &VFXEditorNode::inset_selected);
    ClassDB::bind_method(D_METHOD("delete_selected"), &VFXEditorNode::delete_selected);
    ClassDB::bind_method(D_METHOD("subdivide_selected"), &VFXEditorNode::subdivide_selected);
    ...
    ClassDB::bind_integer_constant(get_class_static(), "", "SELECT_VERTEX", SELECT_VERTEX);
    ClassDB::bind_integer_constant(get_class_static(), "", "SELECT_EDGE", SELECT_EDGE);
    ClassDB::bind_integer_constant(get_class_static(), "", "SELECT_FACE", SELECT_FACE);
}
...
VFXEditorNode::VFXEditorNode() {
    ...
    wireframe_material.instantiate();
    wireframe_material->set_shading_mode(StandardMaterial3D::SHADING_MODE_UNSHADED);
    wireframe_material->set_albedo(Color(0.3f, 0.3f, 0.3f, 0.6f));
    wireframe_material->set_transparency(StandardMaterial3D::TRANSPARENCY_ALPHA);
    wireframe_material->set_cull_mode(StandardMaterial3D::CULL_DISABLED);

    selection_material.instantiate();
    selection_material->set_shading_mode(StandardMaterial3D::SHADING_MODE_UNSHADED);
    selection_material->set_albedo(Color(1.0f, 0.5f, 0.0f, 1.0f));
    selection_material->set_cull_mode(StandardMaterial3D::CULL_DISABLED);
}
...
void VFXEditorNode::_notification(int p_what) {
    if (p_what == NOTIFICATION_ENTER_TREE) {
        ...
        _ensure_wireframe_instance();
        _ensure_selection_highlight();
        _ensure_normal_visual();
    }
    if (p_what == NOTIFICATION_PROCESS) {
        ...
        if ((show_face_normals || show_vertex_normals || show_face_centers) && mesh.is_valid()) {
            _build_normal_visual_mesh();
            if (normal_visual_instance) normal_visual_instance->set_visible(true);
        } else if (normal_visual_instance) {
            normal_visual_instance->set_visible(false);
        }
    }
}
...
void VFXEditorNode::_update_godot_mesh() {
    ...
    if (show_wireframe) _build_wireframe_mesh();
    else if (wireframe_instance) wireframe_instance->set_visible(false);
...
// WIREFRAME OVERLAY
// ============================================================================
void VFXEditorNode::_ensure_wireframe_instance() {
    if (!wireframe_instance) {
        wireframe_instance = memnew(MeshInstance3D);
        wireframe_instance->set_cast_shadows_setting(GeometryInstance3D::SHADOW_CASTING_SETTING_OFF);
        add_child(wireframe_instance);
        wireframe_instance->set_owner(this);
    }
}

void VFXEditorNode::_build_wireframe_mesh() {
    if (!wireframe_instance) _ensure_wireframe_instance();
    if (mesh.is_null()) { wireframe_instance->set_visible(false); return; }
    PackedVector3Array lines = mesh->get_wireframe_lines();
    if (lines.size() < 2) { wireframe_instance->set_visible(false); return; }
    Ref<ArrayMesh> am;
    am.instantiate();
    Array arrays;
    arrays.resize(Mesh::ARRAY_MAX);
    ...
    wireframe_instance->set_mesh(am);
    wireframe_instance->set_material_override(wireframe_material);
    wireframe_instance->set_visible(true);
}

// ============================================================================
// SELECTION HIGHLIGHT
// ============================================================================
void VFXEditorNode::_ensure_selection_highlight() {
    if (!selection_highlight_instance) {
        selection_highlight_instance = memnew(MeshInstance3D);
        selection_highlight_instance->set_cast_shadows_setting(GeometryInstance3D::SHADOW_CASTING_SETTING_OFF);
        add_child(selection_highlight_instance);
        selection_highlight_instance->set_owner(this);
    }
}

void VFXEditorNode::_build_selection_highlight() {
    if (!selection_highlight_instance) _ensure_selection_highlight();
    if (mesh.is_null()) { selection_highlight_instance->set_visible(false); return; }

    PackedVector3Array verts;
    PackedColorArray cols;
    PackedInt32Array idx;
    Color sel_col = Color(1.0f, 0.55f, 0.0f, 1.0f);
    Color hov_col = Color(0.0f, 0.8f, 1.0f, 1.0f);

    if (selected_face >= 0 && selected_face < mesh->get_face_count()) {
        std::vector<vfx::HEVertex*> fverts;
        mesh->get_face_vertices(selected_face, fverts);
        int n = fverts.size();
        for (int i = 0; i < n; i++) {
            int next = (i + 1) % n;
            verts.push_back(fverts[i]->position);
            verts.push_back(fverts[next]->position);
            cols.push_back(sel_col); cols.push_back(sel_col);
        }
    }
    if (hovered_face >= 0 && hovered_face != selected_face && hovered_face < mesh->get_face_count()) {
        std::vector<vfx::HEVertex*> fverts;
        mesh->get_face_vertices(hovered_face, fverts);
        int n = fverts.size();
        for (int i = 0; i < n; i++) {
            int next = (i + 1) % n;
            verts.push_back(fverts[i]->position);
            verts.push_back(fverts[next]->position);
            cols.push_back(hov_col); cols.push_back(hov_col);
        }
    }
    if (selected_vertex >= 0 && selected_vertex < mesh->get_vertex_count()) {
        Vector3 p = mesh->get_vertex_position(selected_vertex);
        _append_box(verts, cols, idx, p, 0.03f, sel_col);
    }
    if (hovered_vertex >= 0 && hovered_vertex != selected_vertex && hovered_vertex < mesh->get_vertex_count()) {
        Vector3 p = mesh->get_vertex_position(hovered_vertex);
        _append_box(verts, cols, idx, p, 0.025f, hov_col);
    }

    if (verts.size() == 0) { selection_highlight_instance->set_visible(false); return; }
    ...
    if (idx.size() > 0) arrays[Mesh::ARRAY_INDEX] = idx;
    if (idx.size() > 0) am->add_surface_from_arrays(Mesh::PRIMITIVE_TRIANGLES, arrays);
    else am->add_surface_from_arrays(Mesh::PRIMITIVE_LINES, arrays);

    selection_highlight_instance->set_mesh(am);
    selection_highlight_instance->set_material_override(selection_material);
    selection_highlight_instance->set_visible(true);
}


// ============================================================================
// NORMAL / FACE CENTER VISUAL OVERLAY
// ============================================================================
void VFXEditorNode::_ensure_normal_visual() {
    if (!normal_visual_instance) {
        normal_visual_instance = memnew(MeshInstance3D);
        normal_visual_instance->set_cast_shadows_setting(GeometryInstance3D::SHADOW_CASTING_SETTING_OFF);
        add_child(normal_visual_instance);
        normal_visual_instance->set_owner(this);
    }
}

void VFXEditorNode::_build_normal_visual_mesh() {
    if (!normal_visual_instance) _ensure_normal_visual();
    if (mesh.is_null()) { normal_visual_instance->set_visible(false); return; }
    ...
    if (show_face_centers) {
        ...
        for (int i = 0; i < fc; i++) {
            ...
            for (auto* v : fv) center += v->position;
            center /= fv.size();
            _append_box_lines(verts, cols, idx, center, 0.02f, col);
        }
    }

    if (verts.size() == 0) { normal_visual_instance->set_visible(false); return; }
    ...
    mat->set_shading_mode(StandardMaterial3D::SHADING_MODE_UNSHADED);
    normal_visual_instance->set_material_override(mat);
    normal_visual_instance->set_mesh(am);
    normal_visual_instance->set_visible(true);
}
...
Transform3D VFXEditorNode::_get_visual_gizmo_transform() const {
    if (gizmo_mode == GIZMO_SCALE) return gizmo_transform;
...
void VFXEditorNode::_update_gizmo_visibility() {
    if (gizmo_node) gizmo_node->set_visible(selected_bone >= 0);
}
...
void VFXEditorNode::_build_gizmo_mesh() {
    ...
    if (gizmo_mode == GIZMO_TRANSLATE) {
        ...
        float ring_r = s * 0.85f, tube = s * 0.02f;
        Color cx = (gizmo_hover_axis == GIZMO_X) ? Color(1.0f, 1.0f, 0.0f) : Color(1.0f, 0.0f, 0.0f);
        Color cy = (gizmo_hover_axis == GIZMO_Y) ? Color(1.0f, 1.0f, 0.0f) : Color(0.0f, 1.0f, 0.0f);
        Color cz = (gizmo_hover_axis == GIZMO_Z) ? Color(1.0f, 1.0f, 0.0f) : Color(0.0f, 0.0f, 1.0f);
...
void VFXEditorNode::set_show_wireframe(bool show) { show_wireframe = show; if (auto_update) _build_wireframe_mesh(); }
bool VFXEditorNode::get_show_wireframe() const { return show_wireframe; }
void VFXEditorNode::set_show_face_normals(bool show) { show_face_normals = show; }
bool VFXEditorNode::get_show_face_normals() const { return show_face_normals; }
void VFXEditorNode::set_show_vertex_normals(bool show) { show_vertex_normals = show; }
bool VFXEditorNode::get_show_vertex_normals() const { return show_vertex_normals; }
void VFXEditorNode::set_show_face_centers(bool show) { show_face_centers = show; }
bool VFXEditorNode::get_show_face_centers() const { return show_face_centers; }

void VFXEditorNode::set_gizmo_mode(int mode) {
    gizmo_mode = mode;
    gizmo_hover_axis = GIZMO_NONE;
    if (gizmo_node && gizmo_node->is_visible()) _build_gizmo_mesh();
}
...
int VFXEditorNode::raycast_gizmo(const Vector3& ray_origin, const Vector3& ray_dir) {
    ...
    if (gizmo_mode == GIZMO_TRANSLATE) {
        ...
        test_axis(GIZMO_Y, Vector3(0, 1, 0));
        test_axis(GIZMO_Z, Vector3(0, 0, 1));
        float box = s * 0.08f, h = box * 0.5f;
        auto test_box = [&](int axis, const Vector3& center) {
            for (int f = 0; f < 6; f++) {
                Vector3 n;
                switch (f) { case 0: n = Vector3( 1, 0, 0); break; case 1: n = Vector3(-1, 0, 0); break;
                    case 2: n = Vector3(0,  1, 0); break; case 3: n = Vector3(0, -1, 0); break;
                    case 4: n = Vector3(0, 0,  1); break; case 5: n = Vector3(0, 0, -1); break; }
...
void VFXEditorNode::gizmo_begin_drag(int axis, const Vector3& ray_origin, const Vector3& ray_dir) {
    ...
    if (gizmo_mode == GIZMO_TRANSLATE) {
        ...
        if (axis <= GIZMO_Z) {
            ...
            if (fabs(axis_vec.dot(cam_dir)) < 0.3f) normal = cam_dir;
            else {
        ...
        } else if (axis == GIZMO_XY) normal = gizmo_transform.basis.xform(Vector3(0, 0, 1)).normalized();
        else if (axis == GIZMO_XZ) normal = gizmo_transform.basis.xform(Vector3(0, 1, 0)).normalized();
        else normal = gizmo_transform.basis.xform(Vector3(1, 0, 0)).normalized();
        gizmo_drag_plane = Plane(normal, origin);
        Vector3 hit;
        if (_ray_vs_plane(ray_origin, ray_dir, gizmo_drag_plane, hit)) gizmo_drag_start_point = hit;
    } else if (gizmo_mode == GIZMO_ROTATE) {
        Vector3 normal;
        ...
        Vector3 cam_dir = ray_dir.normalized();
        gizmo_drag_plane = Plane(cam_dir, origin);
        Vector3 hit;
        if (_ray_vs_plane(ray_origin, ray_dir, gizmo_drag_plane, hit)) {
            gizmo_drag_start_point = hit;
...
void VFXEditorNode::gizmo_drag(const Vector3& ray_origin, const Vector3& ray_dir) {
    ...
    if (gizmo_mode == GIZMO_TRANSLATE) {
        ...
        if (gizmo_drag_axis <= GIZMO_Z) {
            ...
            t.set_origin(t.get_origin() + axis * delta.dot(axis));
...
void VFXEditorNode::gizmo_end_drag() { gizmo_drag_axis = GIZMO_NONE; }
bool VFXEditorNode::is_gizmo_dragging() const { return gizmo_drag_axis != GIZMO_NONE; }
